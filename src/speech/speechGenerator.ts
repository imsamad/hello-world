...
export class SpeechGenerator {
  ...
  static async getSpeechArrayFromTranscriptions({
    ...
    try {
      ...
      for (let i = 0; i < segments.length; i += maxSimultaneousFetch) {
        ...
        // ???: reason to use length=3 condition
  ...
  static async getAudiosSpeakerAndMerge(
    ...
    try {
      const segmentsFromThisSpeaker = segments
        .filter((segment) => segment.speaker === speakerIndex)
        .sort((a, b) => a.begin - b.begin);

      if (segmentsFromThisSpeaker.length === 0) {
        return [];
      }

      // Merge contiguous segments to avoid overlaps and stutters
      const mergedBlocks: { begin: number; end: number }[] = [];
      let currentBlock = {
        begin: segmentsFromThisSpeaker[0].begin,
        end: segmentsFromThisSpeaker[0].end,
      };

      for (let i = 1; i < segmentsFromThisSpeaker.length; i++) {
        const next = segmentsFromThisSpeaker[i];
        // If segments are separated by less than 100ms, consider them contiguous
        if (next.begin <= currentBlock.end + 0.1) {
          currentBlock.end = Math.max(currentBlock.end, next.end);
        } else {
          mergedBlocks.push(currentBlock);
          currentBlock = { begin: next.begin, end: next.end };
        }
      }
      mergedBlocks.push(currentBlock);

      let accumulatedDuration = 0;
      const MAX_CLONING_DURATION = 180; // Limit to 3 minutes of audio for cloning

      for (const block of mergedBlocks) {
        if (accumulatedDuration >= MAX_CLONING_DURATION) break;

        try {
          const start = Math.max(0, block.begin - 0.2);
          const end = block.end + 0.2;
      ...
      console.debug(
        `Accumulated ${accumulatedDuration.toFixed(2)}s of audio for speaker ${speakerIndex} cloning.`,
      );
