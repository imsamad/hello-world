...
export class TextTranslator {
  ...
  static async translateTranscription({
    ...
    let translationPromises: Promise<{ textTranslated: string; totalCost: number }>[] = [];
      ...
      for (let i = 0; i < deepCopyTranscriptions.length; i++) {
        ...
        if (
          ...
          for (let j = 0; j < translations.length; j++) {
            ...
            transcriptionToUpdate.transcription = translations[j].textTranslated;
            transcriptionToUpdate.language = targetLanguage;
            transcriptionToUpdate.cost = translations[j].totalCost;
  ...
  static async getTranslationPromise({
    ...
    do {
      ...
      let { content, totalCost: totalCost_ } = await this.getTranslationPromiseFromAI({
        actualTranscription,
        lastTranscription,
        targetLanguage,
        transcriptionLanguage,
    ...
    return { textTranslated, totalCost };
  }
  ...
  static async translateWithLLM({
    ...
    try {
      const { content, totalCost } = await requestToGPT({
        prompt,
        temperature,
